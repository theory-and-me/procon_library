...
public:
	...
	void make_heavy_light_decomposition(const int root = 0){
		...
		hldorder.resize(num); // hldorder[i]: hld 順序に並べた場合の i 番目の頂点．hld 順序では，heavy-path が必ず区間になっている & 部分木が必ず区間になっている
...
int main(){
    ...
    int root = 0;
    tree.make_heavy_light_decomposition(root);

    cout << tree.order << endl;
    cout << tree.reorder << endl;
    cout << tree.hldorder << endl;

    int x, y;
    cin >> x >> y;

    cout << tree.path(x, y) << endl;
