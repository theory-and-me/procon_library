...
int main(){
    ...
    V<plll> tmp_E(M), E;
    REP(i, M){
        cin >> tmp_E[i].second.first >> tmp_E[i].second.second >> tmp_E[i].first;
        tmp_E[i].second.first--, tmp_E[i].second.second--;
    }

    auto ids = make_vec(M, 0ll);
    iota(ALL(ids), 0);
    sort(ALL(ids), [&tmp_E](ll x, ll y){
        return tmp_E[x] < tmp_E[y];
    });
    ...
    REP(i, N-1){
        ...
        auto [c, tui] = e;
        auto [u, v] = tui;
        // u, v のうち，根から遠い方の頂点の id を辺の id と同一視することで辺クエリを処理する
        if(tree.depth[u]>tree.depth[v]) pos_seg = tree.hldorder[u];
        else pos_seg = tree.hldorder[v];
        init[pos_seg] = c;
    }

    segtree<S, op, e> segtree(init);
    ...
    REP(i, M){
        ...
        ll ma = -INF;
        auto paths = tree.path(u, v, 1); // 1 で辺クエリを指定
        for(auto p: paths){
            chmax(ma, segtree.prod(p.first, p.second+1));
        }   
        cout << tot - ma + c << "\n";
    }
